...
export async function runMixtureOfAgents(prompt: string, config: MoAConfig): Promise<MoAResult> {
  const start = Date.now();
  ...
  const responses = await Promise.all(
    config.models.map((model) =>
      callModel(
        model,
        prompt,
        "You are an expert software engineer. Analyze this problem thoroughly and provide your best solution with reasoning.",
      ),
    ),
  );
  ...
  if (successful.length === 0) {
    return {
      responses,
      synthesis:
        "All models failed to respond. Errors:\n" +
...
${successful.map((r) => `<response model="${r.label}">\n${r.response}\n</response>`).join("\n\n")}
