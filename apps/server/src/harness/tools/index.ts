...
const toolHandlers: Record<
  string,
  (args: Record<string, unknown>, workspaceRoot: string) => Promise<string>
> = {
  Shell: executeShell,
  Read: executeRead,
  Write: executeWrite,
...
async function executeGrep(args: Record<string, unknown>, workspaceRoot: string): Promise<string> {
  ...
  const searchPath = args.path ? resolvePath(args.path as string, workspaceRoot) : workspaceRoot;
...
const TOOL_DEFINITIONS: readonly ToolDefinition[] = [
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        working_directory: {
          type: "string",
          description: "Absolute path to working directory (defaults to workspace root)",
        },
        description: {
          type: "string",
        ...
        block_until_ms: {
          type: "number",
          description:
            "How long to wait for completion in ms (default 30000). Set 0 for background.",
        },
      },
      required: ["command"],
    },
  },
  {
    name: "Read",
    description:
      "Read a file from the filesystem. Returns content with line numbers. Can read images.",
    input_schema: {
      type: "object",
      properties: {
