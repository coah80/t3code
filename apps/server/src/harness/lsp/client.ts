      ...
      try {
        ...
        if (msg.method === "textDocument/publishDiagnostics" && msg.params) {
          ...
          const diags: LspDiagnostic[] = (msg.params.diagnostics ?? []).map((d: any) => ({
            ...
            severity:
              (["", "error", "warning", "info", "hint"] as const)[d.severity ?? 1] ?? "error",
            message: d.message ?? "",
            source: d.source,
          }));
...
export class LspManager {
  ...
  private readonly includeBuiltinServers: boolean;

  constructor(options?: { readonly includeBuiltinServers?: boolean }) {
    this.includeBuiltinServers = options?.includeBuiltinServers ?? true;
  }

  addServer(config: LspServerConfig): void {
    const existingIndex = this.customServers.findIndex((server) => server.id === config.id);
    if (existingIndex >= 0) {
      this.customServers.splice(existingIndex, 1, config);
      return;
    }
  ...
  private findServerForFile(filePath: string): LspServerConfig | null {
    ...
    const allServers = this.includeBuiltinServers
      ? [...this.customServers, ...BUILTIN_SERVERS]
      : [...this.customServers];
  ...
  getToolDefinition(): ToolDefinition {
    return {
      name: "LSP" as any,
      description:
        "Language Server Protocol operations: goToDefinition, hover, getDiagnostics. Provides real-time type errors and navigation.",
      input_schema: {
        type: "object",
        properties: {
          operation: {
          ...
          character: {
            type: "number",
            description: "1-based character position (for goToDefinition, hover)",
          },
        },
