...
import { HarnessAdapter, HarnessAdapterShape } from "../Services/HarnessAdapter.ts";
...
const fakeHarnessAdapter: HarnessAdapterShape = {
  provider: "harness",
  capabilities: { sessionModelSwitch: "in-session" },
  startSession: vi.fn(),
  sendTurn: vi.fn(),
  ...
  Layer.mergeAll(
    Layer.provide(
      ...
      Layer.mergeAll(
        ...
        Layer.succeed(HarnessAdapter, fakeHarnessAdapter),
...
layer("ProviderAdapterRegistryLive", (it) => {
  it.effect("resolves a registered provider adapter", () =>
    Effect.gen(function* () {
      ...
      const harness = yield* registry.getByProvider("harness");
      assert.equal(codex, fakeCodexAdapter);
      assert.equal(claude, fakeClaudeAdapter);
      assert.equal(harness, fakeHarnessAdapter);

      const providers = yield* registry.listProviders();
      assert.deepEqual(providers, ["codex", "claudeAgent", "harness"]);
    }),
  );
