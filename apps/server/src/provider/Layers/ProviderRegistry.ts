...
import { HarnessProviderLive } from "./HarnessProvider";
import type { ClaudeProviderShape } from "../Services/ClaudeProvider";
import { ClaudeProvider } from "../Services/ClaudeProvider";
import type { CodexProviderShape } from "../Services/CodexProvider";
import { CodexProvider } from "../Services/CodexProvider";
import type { HarnessProviderShape } from "../Services/HarnessProvider";
...
const loadProviders = (
  ...
  harnessProvider: HarnessProviderShape,
): Effect.Effect<readonly [ServerProvider, ServerProvider, ServerProvider]> =>
  Effect.all([codexProvider.getSnapshot, claudeProvider.getSnapshot, harnessProvider.getSnapshot], {
...
export const ProviderRegistryLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const harnessProvider = yield* HarnessProvider;
    const changesPubSub = yield* Effect.acquireRelease(
      PubSub.unbounded<ReadonlyArray<ServerProvider>>(),
      PubSub.shutdown,
    );
    const providersRef = yield* Ref.make<ReadonlyArray<ServerProvider>>(
      yield* loadProviders(codexProvider, claudeProvider, harnessProvider),
    );
    ...
    const syncProviders = Effect.fn("syncProviders")(function* (options?: {
      ...
      const providers = yield* loadProviders(codexProvider, claudeProvider, harnessProvider);
    ...
    yield* Stream.runForEach(harnessProvider.streamChanges, () => syncProviders()).pipe(
      Effect.forkScoped,
    );
    ...
    const refresh = Effect.fn("refresh")(function* (provider?: ProviderKind) {
      switch (provider) {
        ...
        case "claudeAgent":
          ...
          break;
        case "harness":
          yield* harnessProvider.refresh;
          break;
        default:
          yield* Effect.all(
            [codexProvider.refresh, claudeProvider.refresh, harnessProvider.refresh],
            {
              concurrency: "unbounded",
            },
          );
  ...
  Layer.provideMerge(HarnessProviderLive),
);
